...
const dbUtils = {
  ...
  getUser: async (login) => {
    try {
      return await connection.execute(`Select * from user where BINARY login = ?;`, [login]);
    } catch (e) {
      console.log(e);
      return null;
    }
  ...
  getUserNickname: async (id) => {
    try {
      return await connection.execute(`Select (nickname) from user where _id = ?;`, [id]);
    } catch {
      return null;
    }
  },
  ...
  initTypes: async (types) => {
    try {
      ...
      for (let t of types) {
        connection.execute('insert into type(_id, name) values(?, ?);', [t.id, t.name]);
      }
    } catch (e) {
  ...
  initCategories: async (categories) => {
    try {
      ...
      for (let c of categories) {
        connection.execute('insert into category(_id, name) values(?, ?);', [c.id, c.name]);
      }
    } catch (e) {
  ...
  createBusiness: async (id) => {
    let bId = uuid.v4();
    try {
      await connection.execute(`insert into business(_id, user_id) values (?, ?);`, [bId, id]);
    } catch {
      return null;
    }
  ...
  createEdition: async function (bId, { data }) {
    ...
    try {
      let result = await connection.execute(
        ...
        [edId, bId, data.category, data.type, data.name, data.description, Date.now()]
      );
  ...
  deletePlan: async (bId) => {
    try {
      return await connection.execute(`delete from business where business._id = ?;`, [bId]);
    } catch {
      return null;
    }
  },
  ...
  getBusinessesByOwnerId: async (owner) => {
    return await connection.execute(`select _id from business where user_id = ?;`, [owner]);
  },
  ...
  getAllEditionsForBusinessesFiltration: async (offset, count, name, sort, category, type) => {
    ...
    return await connection.execute(query, filler);
  },
  ...
  getComments: async (params) => {
    ...
    try {
      ...
      for (let r of result) {
        let [users, fields] = await connection.execute(`select * from user where _id=?;`, [
          r.user_id,
        ]);
        comments.push(
          Object.assign(
  ...
  updateNickname: async (id, nickname) => {
    try {
      return await connection.execute(`update user set nickname = ? where _id = ?;`, [
        nickname,
        id,
      ]);
    } catch {
  ...
  updatePassword: async (id, oldPass, pass) => {
    try {
      let record = await connection.execute(`select password from user where _id = ?;`, [id]);
      let is = await bcrypt.compare(oldPass, record[0][0].password);
      if (!is) return null;
      pass = await bcrypt.hash(pass, salt);
      return await connection.execute(
        `update user set password = ? where _id in (select _id from (select * from user) as tmp where tmp._id = ?);`,
        [pass, id]
      );
    } catch {
      return null;
  ...
  dropToken: async (token) => {
    return await connection.execute(`delete from token where body = ?;`, [token]);
  },
};
